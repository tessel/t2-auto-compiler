...
export async function packageJSON (name) {
  ...
  const request = await got(`https://registry.npmjs.com/${encodedName}`, {
...
export function tarballUrl (name, version, info) {
  ...
  if (!meta) {
    const err = Error(`Unable to find version for ${name}@${version}`)
    err.statusCode = 404
    throw err
  }
...
export async function isBinaryPackage ({ name, version }) {
  try {
    const pkgJson = await packageJSON(name)
    const url = tarballUrl(name, version, pkgJson)
    return checkForGypFileRetry(url)
  } catch (e) {
    if (e.statusCode === 404) {
      return false
    }
